...
export const getAllClassVideos = async (slug: string,filter: ClassVideoQuery = {}) => {
  const params = new URLSearchParams();
  if (filter.search) params.append("search", filter.search);
  if (filter.page) params.append("page", String(filter.page));
  if (filter.limit) params.append("limit", String(filter.limit));

  const res = await axiosInstance.get(`/class-video/contest/${slug}?${params}`);
  return res.data;
};
...
export const CreateClassVideo = async (slug: string, payload: CreateClassVideoInput) => {
  ...
  const res = await axiosInstance.post(`/class-video/contest/${slug}`, formData, {
    headers: {
...
export const UpdateClassVideo = async (id: number, payload: UpdateClassVideoInput) => {
  const formData = new FormData();
