...
const ContestPage: React.FC = () => {
    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Có lỗi xảy ra khi tải danh sách cuộc thi"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : "Xoá thất bại",
        "error"
      );
    } finally {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.successfulDeletes.length > 0) {
          showToast(
            `Đã xóa thành công ${response.successfulDeletes.length} cuộc thi`,
            "success"
          );
        }
      ...
      <Stack spacing={2} sx={{ mb: 3 }}>
        ...
        >
          ...
          {contests.length > 0 && (
            ...
            >
              {selectedContests.length === contests.length
                ? "Bỏ chọn tất cả"
                : "Chọn tất cả"}
            </Button>
          )}
