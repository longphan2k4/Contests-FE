...
export const CreateQuestionPackage = async (payload: CreateQuestionPackageInput) => {
  const res = await axiosInstance.post("/question-packages", payload);
  return res.data;
};

export const UpdateQuestionPackage = async (id: number, payload: UpdateQuestionPackageInput) => {
  const res = await axiosInstance.put(`/question-packages/${id}`, payload);
  return res.data;
};

export const ToggleActive = async (id: number) => {
  const res = await axiosInstance.patch(`/question-packages/${id}/toggle-active`);
  return res.data;
};

export const DeleteQuestionPackages = async (ids: deleteQuestionPackagesType) => {
  const res = await axiosInstance.post("/question-packages/batch-delete", ids);
  return res.data;
};
