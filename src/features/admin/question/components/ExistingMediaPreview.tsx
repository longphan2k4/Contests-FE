...
const ExistingMediaPreview: React.FC<ExistingMediaPreviewProps> = ({
  media,
  onRemove,
}) => {
  ...
  const safeMedia = {
    ...
    id:
      media.id ||
      `media-${media.name}-${Date.now()}-${Math.random()
        .toString(36)
        .substring(2, 10)}`, // Tạo ID duy nhất nếu ID trống
  };
  ...
  const renderFullPreview = () => {
    ...
    return <Typography>Không thể hiển thị nội dung này</Typography>;
  };
