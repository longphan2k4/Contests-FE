      ...
      >
        <Box>
          ...
          <BarChart
            height={300}
            series={[
              { data: studentByClassData.map(d => d.value), label: "Số SV" },
            ]}
            xAxis={[
              { data: studentByClassData.map(d => d.label), scaleType: "band" },
            ]}
          />
        </Box>
        ...
        <Box>
          <Typography variant="h6" gutterBottom>
            Câu hỏi theo chuyên đề
          </Typography>
          <BarChart
            height={300}
            series={[
              {
                data: topicQuestionData.map(d => d.value),
                label: "Số câu hỏi",
              },
            ]}
            xAxis={[
              { data: topicQuestionData.map(d => d.label), scaleType: "band" },
            ]}
          />
        </Box>
        <Box>
          <Typography variant="h6" gutterBottom>
            Số người dùng theo role
          </Typography>
          <BarChart
            height={300}
            series={[
              { data: userRole.map(d => d.value), label: "Số người dùng" },
            ]}
        ...
        <Box>
          ...
          <BarChart
            height={300}
            series={[
              { data: contestsByYear.map(d => d.value), label: "Số cuộc thi" },
            ]}
            xAxis={[
              { data: contestsByYear.map(d => d.label), scaleType: "band" },
            ]}
          />
        </Box>
      </Box>
