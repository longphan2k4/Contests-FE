...
export const getSchools = async (
  ...
  const response = await axiosInstance.get<ApiResponse<SchoolsResponse>>(
    "/school",
    { params }
  );
...
export const updateSchool = async (
  ...
  const response = await axiosInstance.patch<ApiResponse<School>>(
    `/school/${id}`,
    data
  );
  return response.data.data;
...
export const deleteSchool = async (
  ids: number[]
): Promise<DeleteSchoolsResponse> => {
  try {
    const response = await axiosInstance.post<DeleteSchoolsResponse>(
      "/school/delete-many",
      {
        ids,
      }
...
export const deletes = async (id: number): Promise<School> => {
  const response = await axiosInstance.delete<ApiResponse<School>>(
    `/school/${id}`
  );
  return response.data.data;
};
