...
const AnswerContent: React.FC<AnswerContentProps> = ({
  ...
  // ✅ Đảm bảo answermedia là mảng và giới hạn tối đa 4 items
  const safeMedia = Array.isArray(answermedia) 
    ? answermedia.slice(0, 4) 
    : [];
    
  // ✅ Safe access cho answermedia length
    ...
    if (media.type === "audio") {
      return (
        ...
        >
          ...
          <p className="text-sm text-gray-600 mt-2 text-center">Audio giải thích {index + 1}</p>
        </div>
      );
    }
        ...
        {originalMediaCount > 4 && (
          <div className="mt-3 text-center">
            <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
              Hiển thị 4/{originalMediaCount} tài liệu giải thích
            </span>
          </div>
        )}
      </div>
      ...
      <div className="p-6 bg-white rounded-lg shadow-md border transition-all duration-300">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-xl font-semibold text-gray-800">
            Đáp án đúng: <span className="text-green-600 font-bold">{correctAnswer}</span>
          </h2>
        </div>
